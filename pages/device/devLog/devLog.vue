<template>
  <div class="app-container">
    ...
    <div class="main-view">
      <uni-list>
        <uni-list-item
          v-for="log in updateLogList"
          :key="log.id"
          :timestamp="transDate(log.dtUpdate)"
          placement="top"
        >
...
<script>
import { listDevChangeLog } from "@/api/statistic/devChangeLog";
...
export default {
  ...
  onPullDownRefresh() {
    console.log("下拉刷新");
    this.reload = true;
    this.queryParams.pageNum = 1; // 重置页数为1
    this.getList();
  },
  onReachBottom() {
    if (!this.loadAll) {
    ...
    getList() {
      ...
      listDevChangeLog(this.queryParams).then((response) => {
        ...
        if (response.total > 0 && this.queryParams.pageNum <= response.total) {
          console.log("dot");
          this.updateLogList = this.reload
            ? list
            : this.updateLogList.concat(list);
          this.status = "more";
        } else {
          console.log("another");
